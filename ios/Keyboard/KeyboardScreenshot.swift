...
///
/// ⚠️ 平台限制:键盘扩展弹不出相册权限弹窗,授权必须先在主 app 完成(见主 app
/// 的「允许读取截图」按钮)。这里只在已授权(authorized/limited)时读;否则静默返回 nil,
/// 由模型层回退到输入框/剪贴板取材。
...
enum KeyboardScreenshot {
    ...
    /// - 相册已授权(authorized/limited)
    /// - 存在「截图」类型的照片
    /// - 最近一张在 `maxAge` 秒内(用户刚截图就用,隔太久不乱抓)
    /// - 不是上次已自动处理过的那张(去重)
    static func latestFresh(maxAge: TimeInterval = 180) async -> Shot? {
        ...
        guard let image = await requestDownsampled(asset) else { return nil }
